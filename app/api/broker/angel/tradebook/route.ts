/**
 * POST /api/broker/angel/tradebook
 * Get Angel tradebook
 * Internal endpoint - called by /api/v1/tradebook router
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { getTradeBook } = await import('@/lib/angelClient');

    try {
      const trades = await getTradeBook(jwtToken, apiKey);
    ...
    console.error('Error in Angel tradebook:', error);
    return NextResponse.json(
