...
export default function FibonacciTradingChart({
  ...
  const detectSwingPoints = (data: ChartData[]): { swingHigh: SwingPoint | null, swingLow: SwingPoint | null } => {
    ...
    // Use ALL available data (since we're only loading 10 days)
    const recentData = data;

    console.log('[FIB] Finding absolute high/low in', recentData.length, 'candles');
    ...
    <div className="w-full">
      <div className="relative">
        ...
        {indicators.showFibRetracement && swingHigh && swingLow && fibLevels.length > 0 && (
          <div className="absolute top-2 right-2 px-3 py-2 bg-gradient-to-r from-purple-500 to-pink-600 text-white rounded-lg shadow-lg z-50 pointer-events-none">
            <div className="font-bold text-xs">Fibonacci Retracement</div>
            <div className="text-xs mt-1">High: {swingHigh.price.toFixed(2)}</div>
            <div className="text-xs">Low: {swingLow.price.toFixed(2)}</div>
