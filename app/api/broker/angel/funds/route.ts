/**
 * POST /api/broker/angel/funds
 * Get Angel account funds/balance
 * Internal endpoint - called by /api/v1/funds router
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { getFunds } = await import('@/lib/angelClient');

    try {
      const funds = await getFunds(jwtToken, apiKey);
    ...
    console.error('Error in Angel funds:', error);
    return NextResponse.json(
